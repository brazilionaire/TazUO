...
public class MovementImpl : IMovementImpl
{
    ...
    private bool IsOk(Mobile m, bool ignoreDoors, bool ignoreSpellFields, int ourZ, int ourTop, Static[] tiles, List<Item> items)
    {
    ...
    private bool Check(Map.Map map, IPoint3D p, List<Item> items, List<Mobile> mobiles, int x, int y, int startTop, int startZ, bool canSwim, bool cantWalk, out int newZ)
    {
        newZ = 0;
        Mobile m = p as Mobile;

        Static[] tiles = map.Tiles.GetStatics(x, y, true);
        Land landTile = map.Tiles.GetLandTile(x, y);
    ...
    private void GetStartZ(IPoint3D p, Map map, Point3D loc, List<Item> itemList, out int zLow, out int zTop)
    {
        ...
        Static[] Statics = map.Tiles.GetStatics(xCheck, yCheck, true);

        for (int i = 0; i < Statics.Length; ++i)
        {
            Static tile = Statics[i];
